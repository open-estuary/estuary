...
make mrproper
git archive --format=tar --prefix=linux-${kernel_abi}/ ${version} | xz -c > linux-${kernel_abi}.tar.xz
...
sed -i "s/\%define pkg_release.*/\%define pkg_release estuary.${build_num}.fc26/g" kernel.spec
