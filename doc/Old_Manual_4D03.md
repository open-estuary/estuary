...
1. Prepare usb install disk.
    1) Modify estuary/estuarycfg.json. Make sure the platform, distros are all right.
    2) Change the value of "install" to "yes" in object "setup" for usb and the value "device" to your USB install disk.
    (Notice: if the specified usb device does not exist, the first usb device will be selected by default.)
    3) Use build.sh to create the usb install disk.
...
1. Prepare ISO image and install disk.
    1) Modify estuary/estuarycfg.json. Make sure the platform, distros are all right.
    2) Change the value of "install" to "yes" in object "setup" for iso and the value "name" to your target iso image file name.
    3) Use build.sh to create the target install iso image file.
       eg: ./estuary/build.sh -f estuary/estuarycfg.json
    4) Burn the iso image file to DVD disk if you use the physical DVD driver.
